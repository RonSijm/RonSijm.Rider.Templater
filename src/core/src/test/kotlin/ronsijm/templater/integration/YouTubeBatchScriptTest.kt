...
---`
    ...
    fun `test web request to fetch data`() {
        ...
        val context = TestContextFactory.create(fileContent = "test", services = services)
        val parser = TemplateParser(services = services)


        val template = """
            <% tp.web.request("https:
        """.trimIndent()
        ...
        assertTrue(result.contains("delectus aut autem"), "Should contain the title: $result")
    }
    ...
    fun `test tp obsidian request with object literal`() {
        ...
        val services = ServiceContainer(httpService = DefaultHttpService())
        val context = TestContextFactory.create(fileContent = "test", services = services)
        val parser = TemplateParser(services = services)


        val template = """
            <% tp.obsidian.request({ url: "https:
        """.trimIndent()
    ...
    fun `test tp obsidian request with string url`() {
        // This test requires real HTTP - use DefaultHttpService
        val services = ServiceContainer(httpService = DefaultHttpService())
        val context = TestContextFactory.create(fileContent = "test", services = services)
        val parser = TemplateParser(services = services)
