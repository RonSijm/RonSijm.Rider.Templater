...
/**
 * Integration tests for ParallelTemplateParser
 * Tests end-to-end template parsing with the parallel execution infrastructure.
 *
 * Note: Parallel execution is disabled in tests because kotlinx-coroutines is provided
 * by IntelliJ Platform at runtime but not available in the test classpath.
 * These tests verify the ParallelTemplateParser's parsing logic and dependency analysis
 * while using sequential execution.
 */
class ParallelTemplateParserIntegrationTest {

    // Parallel execution is disabled because kotlinx-coroutines is provided by IntelliJ Platform
    // and not available in the test runtime. The parallel scheduling logic is tested separately
    // in DependencyAnalyzerTest and ParallelSchedulerTest.
    ...
    fun `test independent variables can be parallel`() {
        ...
        // These variables are independent and could be executed in parallel
        val template = """
            <%* let x = 10 %>
            <%* let y = 20 %>
            <%* let z = 30 %>
    ...
    fun `test multiple tR blocks maintain order`() {
        ...
        // Using trim to handle whitespace, and checking order
        ...
        // Verify order by checking First comes before Second, and Second before Third
    ...
    fun `test date module with parallel execution`() {
        ...
        // Verify weekday is present (any day name)
        assertTrue(result.contains("Weekday:"))
    }

    // ==================== Parallel vs Sequential Comparison Tests ====================

    @Test
    fun `test sequential execution produces correct results`() {
        // Note: Parallel execution cannot be tested because kotlinx-coroutines is provided
        // by IntelliJ Platform at runtime but not available in the test classpath.
        // The parallel scheduling logic is tested in DependencyAnalyzerTest and ParallelSchedulerTest.
    ...
    fun `test whitespace trimming with parallel execution`() {
        ...
        // The -%> should trim trailing whitespace
